...
impl Receiver {
    ...
    pub fn run(&mut self) -> io::Result<()> {
        ...
        loop {
            ...
            let shard_index = buf[8] as usize;
            let total_shards = buf[9] as usize;

            let payload = buf[10..len].to_vec();

            let entry = self.chunks.entry(chunk_seq).or_insert_with(|| {
                ChunkBuffer {
            ...
            if entry.shards[shard_index].is_none() {
                entry.shards[shard_index] = Some(payload);
            }
            ...
            let header = PacketHeader {
                ...
                session_id,
                seq_no: chunk_seq,
                payload_len: 0, // unknown here
            };

            let aad = header.to_bytes();

            let mut nonce = [0u8; 12];
            nonce[8..12].copy_from_slice(&chunk_seq.to_be_bytes());
            ...
            self.chunks.remove(&chunk_seq);

            println!("Received chunk {}", chunk_seq);
        }
    }
}
